...
//	memset(m_fft, 0, sizeof(m_fft));
	...
	m_Work[0] = 0;
	makewt(m_FFT_SIZE/4, m_Work, m_tSinCos);
	makect(m_FFT_SIZE/4, m_Work, m_tSinCos + m_Work[0]);
	for(int i = 0; i < m_FFT_SIZE; i++){
		pStgBuf[i] = 1.0;
		m_tWindow[i] = (0.5 - 0.5*cos( (PI2*i)/(m_FFT_SIZE-1) ));	//ハニング窓
	}
	m_StgSize = 1;
	m_StgScale = 1.0;
	m_StgK = 0.0;
...
//-------------------------------------------------
// ＦＦＴ処理クラスの再初期化
void __fastcall CFFT::InitFFT(int size)
{
	m_FFT_SIZE = size;
	memset(m_Work, 0, sizeof(int[SQRT_FFT_SIZE+2]));
	makewt(m_FFT_SIZE/4, m_Work, m_tSinCos);
...
void __fastcall CFFT::Calc(double * InBuf, int size, double gain, int stg, int* OutBuf)
{
	...
	double *dp = InBuf;
	for(i=0; i < m_FFT_SIZE; i++, dp++){
		(*dp) *= m_tWindow[i];
	}
	bitrv2(m_FFT_SIZE, m_Work + 2, InBuf);
	cftfsub(m_FFT_SIZE, InBuf, m_tSinCos);
	rftfsub(m_FFT_SIZE, InBuf, m_Work[1], m_tSinCos + m_Work[0]);
...
//	double  d;
	...
	pStgBuf[m_FFT_SIZE/2] = pStgBuf[(m_FFT_SIZE/2) - 2];
}
